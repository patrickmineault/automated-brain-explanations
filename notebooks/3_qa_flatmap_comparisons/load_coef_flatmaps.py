...
def _load_coefs_full(r, subject='S02', qa_questions_version='v3_boostexamples_merged', use_added_wordrate_feature=0):
    ...
    weights, weights_pc = neuro.flatmaps_helper.get_weights_top(args0)
    ...
    if qa_questions_version == 'v3_boostexamples_merged':
        joblib.dump(corrs_test, join(config.PROCESSED_DIR,
...
def _load_coefs_individual(r, subject='S02', qa_questions_version='v3_boostexamples_merged'):
    ...
    args0 = r[r.subject == subject].iloc[0]
    weights = np.array([
        neuro.flatmaps_helper.get_weights_top(r.iloc[i])[0]
        for i in tqdm(range(len(r)))
    ]).squeeze()
...
def _load_coefs_individual_gpt4(rr, subject='S02', use_added_wordrate_feature=0, avg_over_delays=True):
    ...
    weights_list = [
        neuro.flatmaps_helper.get_weights_top(r.iloc[i], avg_over_delays)[0]
        for i in tqdm(range(len(r)))
    ]
...
def _load_coefs_wordrate(rr, subject='S02'):
    ...
    weights = neuro.flatmaps_helper.get_weights_top(r.iloc[0])[0]
